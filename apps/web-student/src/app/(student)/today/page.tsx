...
export default function TodayPage() {
  ...
  const loadDay = useCallback(async () => {
    ...
    try {
      const [myDay, currentUser, allNotifications] = await Promise.all([
        apiFetch<MyDayResponse>('/my-day'), getUser(), apiFetchList<DayNotification>('/notifications').catch(() => []),
      ]);
      setDay(myDay); setUser(currentUser); setNotifications(allNotifications);
    } catch (loadError) {
  ...
  return <div className="mx-auto max-w-[1240px] space-y-6">
    <section className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold capitalize text-indigo-600">{dateLabel}</p><h2 className="mt-1 text-2xl font-bold tracking-[-0.035em] text-slate-950 sm:text-3xl">Мой день{user?.firstName ? `, ${user.firstName}` : ''}</h2></div><p className="text-sm text-slate-500">{timeline.length ? `${timeline.length} ${timeline.length === 1 ? 'пара' : 'пары'} в расписании` : 'Расписание на сегодня'}</p></section>
    <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_360px]"><div className="space-y-6">
      {primary ? <Link href={`/pair-space/${primary.lesson.id}`} className="group block"><section className={`student-panel overflow-hidden p-5 sm:p-7 ${current ? 'border-emerald-200' : 'border-indigo-200'}`}><div className="flex items-start justify-between gap-4"><div><p className={`text-sm font-semibold ${current ? 'text-emerald-700' : 'text-indigo-700'}`}>{current ? 'Сейчас на паре' : 'Следующая пара'}</p><h3 className="mt-5 text-2xl font-bold tracking-[-0.035em] text-slate-950 sm:text-3xl">{primary.lesson.subject}</h3><p className="mt-3 text-lg font-semibold text-slate-800">{time(primary.lesson.startTime)} — {time(primary.lesson.endTime)}</p></div><span className={`hidden shrink-0 rounded-full px-3 py-1.5 text-xs font-semibold sm:inline-flex ${current ? 'bg-emerald-50 text-emerald-700' : 'bg-indigo-50 text-indigo-700'}`}>{current ? `Осталось ${remainingMinutes(current.end, now)} мин` : `Через ${remainingMinutes(primary.start, now)} мин`}</span></div><div className="mt-7 grid gap-3 border-t border-slate-100 pt-5 sm:grid-cols-2"><p className="flex items-center gap-2 text-sm text-slate-600"><Icon name="MapPin" className="h-4 w-4 text-slate-400" />{primary.lesson.room ? `Ауд. ${primary.lesson.room}` : 'Аудитория не указана'}</p><p className="flex items-center gap-2 text-sm text-slate-600"><Icon name="User" className="h-4 w-4 text-slate-400" />{primary.lesson.teacherName || 'Преподаватель не указан'}</p></div>{current ? <div className="mt-5"><div className="h-1.5 overflow-hidden rounded-full bg-emerald-50"><div className="h-full rounded-full bg-emerald-500 transition-all duration-500" style={{ width: `${timeProgress}%` }} /></div></div> : <div className="mt-5 flex items-center gap-2 text-sm font-semibold text-indigo-600">Открыть пространство пары <Icon name="ChevronRight" className="h-4 w-4 transition-transform group-hover:translate-x-1" /></div>}</section></Link> : <section className="student-panel"><EmptyState icon={<Icon name="CalendarDays" className="h-8 w-8" />} title="На сегодня пар нет" description="Свободный день — можно спокойно заняться своими делами." action={<Link href="/week" className="rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-indigo-700">Открыть расписание</Link>} /></section>}
